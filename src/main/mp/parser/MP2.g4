...
def emit(self):
    ...
    if self.type == self.ILLEGAL_ESCAPE:
        raise IllegalEscape(result.text[1:len(result.text)-1])
...
array_type
	: ARRAY LEFTSQUAREBRACKET expression DOUBLEDOT expression RIGHTSQUAREBRACKET OF primitive_type
	;
...
assignment_statement
    : ((IDENTIFIERS | index_expression) ASSIGN_OP)+  expression SEMICOLON
    ;
